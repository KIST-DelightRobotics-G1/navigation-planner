...
struct LabeledCloud {
    ...
    // Ground-plane normal (robot frame, points up) fitted from this cloud, so the
    // grid builder can level BOTH clouds by the true sensor tilt. Default = up /
    // invalid (no correction). See ground_plane.hpp.
    float ground_nx = 0.f, ground_ny = 0.f, ground_nz = 1.f;
