...
struct CostmapConfig {
    ...
    // Noise removal: an occupied cell needs at least this many occupied 8-
    // neighbours to count as an obstacle. Sparse far-LiDAR returns (isolated
    // cells) otherwise inflate into lethal disks that wall off free space; walls
    // are dense and survive. 0 = off. Raise to prune more, lower to keep small
    // real obstacles (safety trade-off).
