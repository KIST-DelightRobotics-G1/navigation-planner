...
Costmap CostmapBuilder::build(const OccupancyGrid& g, const GridConfig& gcfg,
    ...
    // Noise removal: drop occupied cells with too few occupied neighbours —
    // isolated far-LiDAR returns each inflate into a lethal disk and wall off
    // traversable space, while real walls are dense and survive. Neighbour counts
    // use the pre-filter snapshot so the removal doesn't cascade.
    if (cfg.obstacle_min_neighbors > 0) {
        const std::vector<uint8_t> src = obst;
        for (int iy = 0; iy < g.n; ++iy)
            for (int ix = 0; ix < g.n; ++ix) {
                if (!src[size_t(iy) * g.n + ix]) continue;
                int cnt = 0;
                for (int dy = -1; dy <= 1; ++dy)
                    for (int dx = -1; dx <= 1; ++dx) {
                        ...
                        cnt += src[size_t(ny) * g.n + nx];
                    }
                if (cnt < cfg.obstacle_min_neighbors) obst[size_t(iy) * g.n + ix] = 0;
            }
    }
