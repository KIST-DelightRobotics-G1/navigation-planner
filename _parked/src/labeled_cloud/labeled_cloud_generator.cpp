...
void LabeledCloudGenerator::run() {
    ...
    while (running_) {
        ...
        if (d && !d->empty() && d->stamp_ns != last_stamp) {
            ...
            // Fit the floor from this cloud (robot frame, before any band filter)
            // -> its normal is the true sensor tilt for the grid builder to level
            // both clouds by. Temporal prior smooths gait jitter / covers dropouts.
            prev_ground_ = fit_ground_plane(robot.xyz, ground_cfg_, prev_ground_);
            robot.ground_nx = prev_ground_.nx;
            robot.ground_ny = prev_ground_.ny;
            robot.ground_nz = prev_ground_.nz;
            robot.ground_valid = prev_ground_.valid;
